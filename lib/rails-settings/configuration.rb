module RailsSettings
  class Configuration
    ...
    def key(name, options={})
      ...
      raise ArgumentError.new("has_settings: Option :defaults expected, but got #{options.keys.join(', ')}") unless options.blank? || (options.keys == [:defaults,:class_name]) || (options.keys == [:class_name,:defaults]) || (options.keys == [:class_name]) || (options.keys == [:defaults])
